...
mod tests {
    ...
    fn remove() {
        ...
        set.insert(Range { start: 1, end: 1 }).unwrap();
        set.insert(Range { start: 3, end: 3 }).unwrap();
        assert_eq!(set.len(), 2);
        set.remove(1);
        assert_eq!(set.len(), 1);
    }
    ...
    fn remove_range() {
        let mut set = RangeSet::new();
        set.insert(Range::new(1, 10)).unwrap();

        set.remove_range(Range::new(4, 7));
        assert_eq!(set.len(), 2);
        assert_eq!(set.get(0), Some(Range::new(1, 3)));
        assert_eq!(set.get(1), Some(Range::new(8, 10)));

        set.insert(Range::new(100, 200)).unwrap();
        assert_eq!(set.len(), 3);
        set.remove_range(Range::new(110, 250));
        assert_eq!(set.len(), 3);
        assert_eq!(set.get(2), Some(Range::new(100, 109)));
    }

    #[test_case]
    fn insert_and_merge() {
        let mut set = RangeSet::new();
        set.insert(Range { start: 1, end: 10 }).unwrap();
        set.insert(Range { start: 11, end: 20 }).unwrap();
        assert_eq!(set.len(), 1);
        assert_eq!(set.get(0), Some(Range::new(1, 20)));
    }
}
